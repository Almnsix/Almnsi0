...
// 1. توكن بوت تيليجرام (من @BotFather)
$API_KEY = '8080542644:AAEqZVvt7UOVCUjW-CAuQNSLfV0PDs7o6eY';

// 2. تعريف التوكن كمتغير
$BOT_TOKEN = $API_KEY;

echo "https://api.telegram.org/bot" . $BOT_TOKEN . "/setwebhook?url=https://" . $_SERVER['SERVER_NAME'] . "" . $_SERVER['SCRIPT_NAME'];

function bot($method, $datas = []){
    global $BOT_TOKEN; // إضافة هذا السطر مهم
...
// 3. أيدي حسابك في تيليجرام
$admin = 1086791687; // تم تصحيح السطر
...
if($chat_id == $admin){
        ...
        bot('sendMessage',[
            ...
            'reply_markup' => json_encode([
                'inline_keyboard' => [
                    ...
                    [['text' => '- حذف مجلد 🗑؛','callback_data' => 'deleteD'],['text' => '- انشاء مجلد 🕸،','callback_data' => 'uploadD']],
    ...
    if($files['mode'] == 'upload'){
        if($message->document){
            global $BOT_TOKEN; // إضافة هنا أيضاً
            $url = 'https://api.telegram.org/file/bot' . $BOT_TOKEN . '/' . bot('getFile',['file_id' => $message->document->file_id])->result->file_path;
            $files['url'] = $url;
            bot('sendMessage',[
                'chat_id' => $chat_id,
